...
export function ManualItemForm({ allProducts, existingProductIds, onAdd }) {
  ...
  return _jsxs('form', {
    ...
    children: [
      _jsx('p', {
        style: { margin: 0, fontWeight: 600, fontSize: fontSize.base, color: colors.text },
      ...
      _jsxs('select', {
        ...
        children: [
          _jsx('option', { value: '', children: 'Seleccionar producto...' }),
          available.map((p) =>
            _jsxs('option', { value: p.id, children: [p.name, ' (', p.unitLabel, ')'] }, p.id)
          ),
        ],
      }),
      _jsx('input', {
